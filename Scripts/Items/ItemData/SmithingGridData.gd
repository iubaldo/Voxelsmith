...
export onready var componentType: ComponentType # what kind of component the grid is using, defined by the pattern used
...
func setSubvoxelColor(newColor: Color) -> void:
	for row in gridMatrix:
		for vox in row:
			if vox:
				vox.subvoxelColor = newColor
				vox.setSubvoxelColor()
	return
